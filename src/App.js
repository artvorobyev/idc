  ...
  render() {
    ...
    if (params.page === 'banner'){
      ...
      return (
        ...
        <h1 className="App__title">inequality<br/> and diversity<br/> conference</h1>
        </header>
        ...
        options={[
          ...
          className={`mt-3 ${ (_self.state.format.value !== 'speaker' ) ? 'd-none' : '' } `}
          onChange={_self.inputChange.bind(_self)}
          />
          <Input 
          name="description"
          ...
          <Upload 
          className={`mt-3 ${ (imageFormats.includes(_self.state.format.value)) ? '' : 'd-none' }`}
          handler={_self.addedImage.bind(_self)}
          >Upload Image</Upload>

          <Button loading={_self.state.loading} className="mt-4">Generate</Button>
