...
class App extends React.Component {
  ...
  formSubmit(event) {
    ...
    fetch('https://base64.artvorobyev.ru', {
      method: 'POST',
      headers: {
      ...
      .then((response) => {
        ...
        return fetch('https://idc-backend.herokuapp.com/screenshot', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json; charset=UTF-8',
            'Access-Control-Allow-Origin': '*',
      ...
      return (
        <div className='App'>
          <div className='App__screen'>
            <div className='App__banner' style={{ zoom: state.zoom }}>
              <Banner imageFormats={imageFormats} {...state} />
            </div>
          </div>
          <div className='App__sidebar'>
            ...
            <form className='App__form' onSubmit={_self.formSubmit.bind(_self)}>
              ...
              <Input
                ...
                value={state.role}
                className={`mt-3 ${
                  state.format.value !== 'speaker' ? 'd-none' : ''
                } `}
                onChange={_self.inputChange.bind(_self)}
              />
              <Input
              ...
              <Upload
                className={`mt-3 ${
                  imageFormats.includes(state.format.value) ? '' : 'd-none'
                }`}
              ...
              <Button loading={state.loading} className='mt-4'>
                Generate
              </Button>
            </form>
          </div>
